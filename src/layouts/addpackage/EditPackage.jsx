...
// Add this sorting function at the top of your component
...
const EditPackage = () => {
  ...
  console.log(maxItineraries);
  console.log(packageData?.Itinaries);
  ...
  const handleAddServiceEx = () => {
    if (!serviceInputEx || serviceInputEx.trim() === "") {
      toast.error("Please enter an exclude service before adding", {
    ...
    // If validation passes, add the service
  ...
  const handleDeleteServiceEx = (index) => {
    if (servicesEx.length <= 1) {
      toast.error(
        "Cannot delete all exclude services. At least one is required.",
        {
          position: "bottom-right",
          autoClose: 3000,
          hideProgressBar: false,
  ...
  const handleDaysSelect = (e) => {
    ...
    setMaxItineraries(Math.max(newDays, selectedNights));
  ...
  const handleNightsSelect = (e) => {
    ...
    setMaxItineraries(Math.max(selectedDays, newNights));
    setValuesChanged(true);
  };
  ...
  const handleUpdatePackage = async (e) => {
    ...
    if (!selectplacetovisit) {
      setErrorMessage("Please Select Place To Visit!");
      openErrorSB();
      return;
    }
    ...
    if (selectedHotelTypes?.length < 1) {
      setErrorMessage("Please Select Hotel Types");
      openErrorSB();
      return;
    }

    if (!roomSharing) {
      setErrorMessage("Please Select Roomsharing");
      openErrorSB();
      return;
    }

    if (mealValue?.length < 1) {
      setErrorMessage("Please Select Meal Required");
      openErrorSB();
      return;
    }

    if (destCatValue?.length < 1) {
      setErrorMessage("Please Select Destination Category");
      openErrorSB();
      return;
    }

    if (Number(selectedDays) === 0) {
      setErrorMessage("Please Select Total Travelling Days");
      openErrorSB();
      return;
    }

    if (Number(selectedNights) === 0) {
      setErrorMessage("Please Select Total Travelling Nights");
      openErrorSB();
      return;
    }

    if (!selectedMealType) {
      setErrorMessage("Please Select Meal Type");
      openErrorSB();
      return;
    }

    if (!selectedTravelBy) {
      setErrorMessage("Please Select Travel By");
      openErrorSB();
      return;
    }

    if (services?.length < 1) {
      setErrorMessage("Please Enter Include Services");
      openErrorSB();
      return;
    }

    if (servicesEx?.length < 1) {
      setErrorMessage("Please Enter Exclude Services");
      openErrorSB();
      return;
    }

    if (!moreDetails) {
      setErrorMessage("Please Enter More Details");
      openErrorSB();
      return;
    }

    if (!sightseeing) {
      setErrorMessage("Please Enter Sightseeing");
      openErrorSB();
      return;
    }

    if (priceDetails?.length < 1) {
      setErrorMessage("Please Price Per Person For Adult, Children & Infant");
      openErrorSB();
      return;
    }
  ...
  const handleAddItinerary = () => {
    const maxAllowed = Math.max(Number(selectedDays), Number(selectedNights));
    const currentCount = packageData?.Itinaries?.length || 0;

    if (currentCount >= maxAllowed) {
      setErrorMessage(`Cannot add more than ${maxAllowed} itineraries`);
      openErrorSB();
      return;
    }
  ...
  const [initialNights, setInitialNights] = useState(0);
  ...
  const packageDetail = async () => {
    ...
    const sortedPriceDetails = sortDatesByMonth(allData?.price_and_date);
    ...
    setInitialDays(Number(allData?.total_days));
    setInitialNights(Number(allData?.total_nights));
    setMaxItineraries(
      Math.max(Number(allData?.total_days), Number(allData?.total_nights))
    );
  };
  ...
  const handleAddPriceDetail = () => {
    ...
    if (Number(pricePerPerson) < Number(pricePerPersonForChild)) {
      setErrorMessage("Childrens Price cannot be bigger than Adult Price");
      openErrorSB();
      return;
    }

    if (Number(pricePerPerson) < Number(pricePerPersonForInfant)) {
      setErrorMessage("Infants Price cannot be bigger than Adult Price");
      openErrorSB();
      return;
    }

    if (Number(pricePerPersonForChild) < Number(pricePerPersonForInfant)) {
      setErrorMessage("Infants Price cannot be bigger than Children's Price");
      openErrorSB();
      return;
    }
    ...
    const newDetail = {
      ...
      infant_price: Number(pricePerPersonForInfant),
        ...
        <TabContext value={value}>
                                ...
                                {services?.map((service, index) => (
                                  ...
                                  >
                                    <ListItemText
                                      primary={
                                        <Typography>
                                          {expandedServices[`service_${index}`]
                                            ? service
                              ...
                              <FormControl fullWidth variant="outlined">
                                <Select
                                  ...
                                  style={{
                                    ...
                                    border: "0px solid #dadbda", // Border to match the input field design
                                  }}
                                  label="Package Type"
                                >
                                  <MenuItem value="">Select</MenuItem>
          ...
          <TabPanel value="2">
            <MDBox pt={2} pb={1}>
                  ...
                  packageData?.Itinaries.map((hotel) => (
                    <Grid item xs={12} md={6} xl={4} key={hotel.id}>
                      <Card sx={{ position: "relative", paddingTop: "10px" }}>
                        {" "}
                        {/* Set the card's position to relative */}
                        <CardMedia
                          component="img"
                          image={hotel.photo}
                          alt={hotel.title}
                          ...
                          <>
                            <Typography variant="body2" color="textSecondary">
                              {expandedItems[hotel._id]
                                ? hotel.activity.replace(/<[^>]+>/g, "")
                                : hotel.activity
                                    .replace(/<[^>]+>/g, "")
                                    .substring(0, 250) + "..."}
                            </Typography>
                            {hotel.activity.replace(/<[^>]+>/g, "").length >
                              250 && (
                              <Button
                          ...
                          >
                            ...
                            >
                              <Typography
                                ...
                                sx={{ color: "#7b809a" }}
                              >
                                {hotel.selected_rooms[0].room_type}
                              </Typography>
                              <Typography
                                variant="body2"
                                component="div"
              ...
              {packageData?.Itinaries?.length < maxItineraries &&
                packageData?.Itinaries?.length <
                  Math.max(selectedDays, selectedNights) && (
                  <Button
                    variant="contained"
                    color="primary"
                    style={{
